...
class UserService {
  ...
  Future<List<StudyMaterial>> getTodayStudyMaterials(
    ...
    try {
      ...
      // 今日のstudySessionをクエリで取得
      QuerySnapshot studySessionSnapshot = await firestore
          .collection('users')
          .doc(userId)
      ...
      if (studySessionSnapshot.docs.isEmpty) {
        return []; // 勉強記録がない場合は空リストを返す
      }

      // studySessionのbookIdを収集（重複を排除）
      Set<String> bookIds = studySessionSnapshot.docs
          .map((doc) => doc['bookId'] as String)
          .toSet();

      // privateBooksから必要な本のデータを取得
      Map<String, Book> bookMap = {};
      for (String bookId in bookIds) {
        DocumentSnapshot bookSnapshot = await firestore
            .collection('users')
            .doc(userId)
        ...
        if (bookSnapshot.exists) {
          bookMap[bookId] =
              Book.fromFirestore(bookSnapshot.data() as Map<String, dynamic>);
        }
      }

      // StudyMaterialオブジェクトのリストを生成
      List<StudyMaterial> studyMaterials = studySessionSnapshot.docs.map((doc) {
        Map<String, dynamic> sessionData = doc.data() as Map<String, dynamic>;
        String bookId = sessionData['bookId'];
        Book? book = bookMap[bookId];

        if (book == null) {
          // 対応する本が見つからない場合、デフォルトのデータを設定
          book = Book(
            id: bookId,
            imgUrl: '',
            title: 'Unknown Book',
      ...
      print('Error fetching today\'s study materials: $e');
  ...
  Future<void> addCommentWithNotification({
    ...
    try {
      ...
      DocumentSnapshot dailyGoalSnap =
          await _firestore.collection('DailySummary').doc(dailyGoalId).get();
